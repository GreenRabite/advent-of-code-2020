...
#  While these documents are extremely similar, North Pole Credentials aren't issued by a country and therefore aren't actually 
...
def valid_birth_year?(input)
  input.to_i >= 1920 && input.to_i <= 2002
end

def valid_issue_year?(input)
  input.to_i >= 2010 && input.to_i <= 2020
end

def valid_expiration_year?(input)
  input.to_i >= 2020 && input.to_i <= 2030
end

def valid_height?(input)
  if input.include?("in")
    input.to_i >= 59 && input.to_i <= 76
  elsif input.include?('cm')
    input.to_i >= 150 && input.to_i <= 193
  else
    false
  end
end
...
file = File.read("day04/input.txt")

passport_hash = {}
valid_passports = 0

file.split(/\n/).each do |line|
  # Reset passport hash if end of passport
  unless line.length > 0
    valid_passports +=1 if passport_valid?(passport_hash)
    passport_hash = {}
    next
  end

  line.split(' ').each do |data_field|
    field, data = data_field.split(':')
    passport_hash[field] = data
  end
end

# Need to check last passport
valid_passports +=1 if passport_hash != {} && passport_valid?(passport_hash)
