...
# Their password database seems to be a little corrupted: some of the passwords wouldn't have been allowed by the Official Toboggan Corporate 
...
# a given letter must appear for the password to be valid. For example, 1-3 a means that the password must contain a at least 1 time and 
# at most 3 times.
...
# results = file.reduce(0) do |accum, line|
#   freq, letter, password = line.split(' ')
#   minFreq, maxFreq = freq.split('-').map(&:to_i)
#   accum += 1 if password.count(letter.chars.first) >= minFreq && password.count(letter) <= maxFreq
#   accum
# end
...
# rules from his old job at the sled rental place down the street! The Official Toboggan Corporate Policy actually works a little differently.
...
resultsB = file.reduce(0) do |accum, line|
  freq, letter, password = line.split(' ')
  onIdx, offIdx = freq.split('-').map(&:to_i)
  first_one = password[onIdx - 1] == letter.chars.first 
  second_one = password[offIdx - 1] == letter.chars.first
  accum += 1 if first_one ^ second_one
  accum
end
