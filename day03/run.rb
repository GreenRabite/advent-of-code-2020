...
# in trees. You'll need to see which angles will take you near 
# the fewest trees.
...
# cheaper model that prefers rational numbers); start by counting
...
class Map
  attr_accessor :map, :count, :current_col, :current_row
  ...
  def self.create_map
    file = File.read("day03/input.txt")
    rows = file.split(/\n/)
    map = Array.new(rows.length) { Array.new(rows.first.length) }
    rows.each_with_index do |row, row_idx|
      row.chars.each_with_index do |terrain, col_idx|
        map[row_idx][col_idx] = terrain
      end
    end
  end

  def mark_map(row_idx, col_idx)
    # wraps the columns if needed
    @current_col = @current_col % @map.first.length
    @count +=1 if @map[@current_row][@current_col] == '#'
  end

  def run(slope_col, slope_row)
    while @current_col <= @map.length
      # puts "Current Row is: #{@current_row}"
      # puts "Current Col is: #{@current_col}"
      @current_row += slope_row
      @current_col += slope_col
      break if @current_row >= @map.length
      mark_map(@current_row, @current_col)
    end
    @count
  end
end
...
# Part B
slope_2 = Map.new.run(1,1)
slope_3 = Map.new.run(5,1)
slope_4 = Map.new.run(7,1)
slope_5 = Map.new.run(1,2)

puts slope_1 * slope_2 * slope_3 * slope_4 * slope_5
