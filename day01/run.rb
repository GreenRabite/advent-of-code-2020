...
# little picture of a starfish; the locals just call them stars. None of the currency exchanges seem to have heard of them,
...
numbers = file.map(&:to_i).sort

# Part A
lookup = numbers.each_with_object({}) do |ele, hash|
  hash[ele] = true
end
...
numbers.each do |num|
  numbers.reverse.each do |rev|
    if lookup[2020 - num - rev]
      puts num * rev * (2020 - num - rev)
      return
    end
  end
end